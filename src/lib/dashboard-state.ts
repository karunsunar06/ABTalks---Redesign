...
export type DashboardMock = {
  ...
  greeting: { headline: string; sub: string };
  ...
  achievements: { name: string; note: string; icon: "rocket" | "flame" | "globe" | "sparkles" }[];
  achievementsUnlocked: number;
  recent?: { name: string; note: string };
};
...
export const DASHBOARD_STATES: Record<DashboardStateKey, DashboardMock> = {
  active: {
    key: "active",
    greeting: { headline: "Builder.", sub: "You've got one thing to ship today." },
    streak: 7,
    ...
    recent: { name: "First Project Shipped", note: "Completed your first real-world build." },
  },
  "first-day": {
    key: "first-day",
    greeting: { headline: "Builder.", sub: "Your journey starts here." },
    notice: {
    ...
    skills: [
      ...
      { name: "Prompt Engineering", level: "Not started", value: 0 },
      { name: "Machine Learning", level: "Not started", value: 0 },
    ],
    achievements: [],
  ...
  missed: {
    key: "missed",
    greeting: { headline: "Builder.", sub: "Today is a good day to pick it back up." },
    notice: {
    ...
    recent: { name: "Public Builder", note: "You shared your 10th project." },
  },
};

/**
 * Dev-only state switching: /dashboard?state=first-day | missed.
...
export function useDashboardMock(): DashboardMock {
  ...
  useEffect(() => {
    ...
    if (value === "first-day" || value === "missed" || value === "active") setKey(value);
  }, []);
